...
def parse_calibration_settings_file(filename):
    ...
    calibration_settings['camera0'] = int(os.getenv('CAM_ID_0'))
    calibration_settings['camera1'] = int(os.getenv('CAM_ID_1'))
...
def calibrate_camera_for_intrinsic_parameters(images_prefix):
    ...
    for i, frame in enumerate(images):
        ...
        if ret == True:
            ...
            if calibration_settings['check_images']:
                view_resize = calibration_settings['view_resize']
                frame_small = cv.resize(
                    frame, None, fx=1/view_resize, fy=1/view_resize)
                        
                cv.drawChessboardCorners(frame_small, (rows, columns), corners / view_resize, ret)
                cv.putText(frame_small, 'If detected points are poor, press "s" to skip this sample',
                        (25, 25), cv.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)

                cv.imshow('img', frame_small)
                
                ret, cmtx, dist, rvecs, tvecs = cv.calibrateCamera(
                    [objp], [corners], (width, height), None, None)
                print(ret)
...
def stereo_calibrate(mtx0, dist0, mtx1, dist1, frames_prefix_c0, frames_prefix_c1):
    ...
    for frame0, frame1 in zip(c0_images, c1_images):
        ...
        if c_ret1 == True and c_ret2 == True:
            ...
            view_resize = calibration_settings['view_resize']
            p0_c1 = (corners1[0, 0] / view_resize).astype(np.int32)
            p0_c2 = (corners2[0, 0] / view_resize).astype(np.int32)

            if calibration_settings['check_stereo_images']:
                frame_small0 = cv.resize(
                    frame0, None, fx=1/view_resize, fy=1/view_resize)
                frame_small1 = cv.resize(
                    frame1, None, fx=1/view_resize, fy=1/view_resize)
                cv.putText(frame_small0, 'O', (p0_c1[0], p0_c1[1]),
                        cv.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
                cv.drawChessboardCorners(frame_small0, (rows, columns), corners1 / view_resize, c_ret1)
                cv.imshow('img', frame_small0)

                cv.putText(frame_small1, 'O', (p0_c2[0], p0_c2[1]),
                        cv.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 1)
                cv.drawChessboardCorners(frame_small1, (rows, columns), corners2 / view_resize, c_ret2)
                cv.imshow('img2', frame_small1)

                stereocalibration_flags = cv.CALIB_FIX_INTRINSIC
                ret, CM1, dist0, CM2, dist1, R, T, E, F = cv.stereoCalibrate([objp], [corners1], [corners2], mtx0, dist0,
...
if __name__ == '__main__':
    ...
    if calibration_settings['individual_calibration']:
    ...
    if calibration_settings['stereo_calibration']:
    ...
    # R, T = stereo_calibrate(cmtx1, dist1, cmtx0, dist0, frames_prefix_c1, frames_prefix_c0)
    ...
    shape = calibration_settings['frame_width'], calibration_settings['frame_height']
    rect_l, rect_r, proj_mat_l, proj_mat_r, Q, roiL, roiR= cv.stereoRectify(cmtx0, dist0, cmtx1, dist1, shape, R1, T1, 1 ,(0,0))
    Left_Stereo_Map= cv.initUndistortRectifyMap(cmtx0, dist0, rect_l, proj_mat_l,
                                             shape, cv.CV_16SC2)
    Right_Stereo_Map= cv.initUndistortRectifyMap(cmtx1, dist1, rect_r, proj_mat_r,
                                              shape, cv.CV_16SC2)
    
    print("Saving parameters ......")
    cv_file = cv.FileStorage("stereo_rectify_maps.xml", cv.FILE_STORAGE_WRITE)
